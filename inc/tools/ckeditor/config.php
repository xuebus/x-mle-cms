...
function CheckAuthentication(){
	...
	//是否允许上传，登录检测
	$SESS_SAVE_PATH = str_replace('\\','/',realpath('../../../../../../inc/tmp/sessions/'));
	is_writeable($SESS_SAVE_PATH) && is_readable($SESS_SAVE_PATH) && session_save_path($SESS_SAVE_PATH);
	@session_start();
	return $_SESSION['admin']['login']['condition'] == 'mlecms' ? true : false;
}
...
//上传成功后返回的图片地址前缀
// 获取相对根路径,注意文件夹名区分大小写 $baseUrl = '/cms/uploadfile/CKEditor/';
//(CKEditor文件夹所在的网站根目录下的第一个文件夹名以前截取)
//$baseUrl = '/ckfinder/userfiles/';
$baseUrl = $_SERVER['PHP_SELF'] ? $_SERVER['PHP_SELF'] : $_SERVER['SCRIPT_NAME'];
$baseUrl = substr($baseUrl,0,strpos($baseUrl,'inc')).'inc/uploads/ckeditor/';
...
*/
//上传文件保存目录
//$baseDir = 'E:/Home/uploadfile/CKEditor/';
//$baseDir = resolveUrl($baseUrl);
//获取物理路径(Fckeditor文件夹所在的网站根目录下的第一个文件夹名以前截取)
$baseDir = __FILE__;
$baseDir = substr($baseDir,0,strpos($baseDir,'inc')).'inc/uploads/ckeditor/';
...
$config['ResourceType'][] = Array(
		...
		'url' => $baseUrl,
		'directory' => $baseDir,
		'maxSize' => 0, //文件、附件上传大小限制，单位KB，0不限制
...
$config['ResourceType'][] = Array(
		...
		'url' => $baseUrl,
		'directory' => $baseDir,
		'maxSize' => 0, //Flash上传大小限制，单位KB，0不限制
