...
<div class="classify">{:foreach category::data(4,$mle['channel_id'],0,1) as $n:}<a href="{:$n['URL']:}">{:$n['title']:}</a>{:if $n['next_level']:}&nbsp;|&nbsp;{:/if:}{:/foreach:}</div>
<div class="box">
	<div class="main">
		<div class="titlebar">
			<ol class="title">{:$mle['channel_title']:}</ol>
			<ol class="location">{:$lang['page']['location']:}<a href="./">{:$lang['page']['home']:}</a> &gt;&gt; {:$mle['channel_title']:}</ol>
		</div>
		
		<!-- 列表数据，全部分类 -->
		{:foreach download::data(0,10,0,$mle['channel_id'],0,0,0,0,0,1,0,0,3) as $d:}
		<div class="data">
	...
	<div class="sidebar">
		...
		<div class="well">
			{:foreach download::data(0,10,1,$mle['channel_id']) as $i => $d:}
			<ul>
				<li class="num">{:$i + 1:}</li>
		...
		<div class="well">
			{:foreach download::data(9,10,0,$mle['channel_id']) as $i => $d:}
			<ul>
				<li class="num">{:$i + 1:}</li>
