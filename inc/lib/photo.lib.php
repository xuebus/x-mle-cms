...
class photo{
	public static function data(){
		...
		if(empty($args[5])){
			global $db;
			$dbprefix = $db->prefix;
			$query_sql = "SELECT a.*,b.`title` as `channel_title`,b.`pathcategory`,b.`pathcontent`,c.`title` as `category_title` FROM `{$dbprefix}picture` a LEFT JOIN `{$dbprefix}channel` b ON a.`channel` = b.`id` LEFT JOIN `{$dbprefix}category` c ON a.`category` = c.`nexus` ";
			$count_sql = "SELECT count(*) FROM `{$dbprefix}picture` a "; 
		...
		foreach($result as &$n){
			if(isset($n['id'])){ 
				...
				$n['description'] = explode(STRING_DELIMITED,$n['description']); 
	...
	private static function single($id,$related = 0){
		global $db;
		$sql_public = "SELECT a.*,b.`title` as `channel_title`,b.`pathcategory`,b.`pathcontent`,c.`title` as `category_title` FROM `{$db->prefix}picture` a LEFT JOIN `{$db->prefix}channel` b ON a.`channel` = b.`id` LEFT JOIN `{$db->prefix}category` c ON a.`category` = c.`nexus` WHERE a.`lang` = '".LANG."' && a.`audit` = 1 && a.`recycle` = 0 "; 
		$result = $db->query($sql_public . "&& a.`".(is_numeric($id) ? 'id' : 'aid')."` = '{$id}' LIMIT 1"); 
		if($related == 1 && is_numeric($id) && isset($result[0]['id'])){ 
