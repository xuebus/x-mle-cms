...
class download{
	public static function data(){
		...
		if($args[5] == 0){
			global $db;
			$query_sql = "SELECT a.*,b.`title` as `channel_title`,b.`pathcategory`,b.`pathcontent`,c.`title` as `category_title` FROM `{$db->prefix}download` a LEFT JOIN `{$db->prefix}channel` b ON a.`channel` = b.`id` LEFT JOIN `{$db->prefix}category` c ON a.`category` = c.`nexus` ";
			$count_sql = "SELECT count(*) FROM `{$db->prefix}download` a "; 
			$sql = "WHERE a.`lang` = '".LANG."' && a.`audit` = 1 && a.`recycle` = 0 "; 
			...
			switch($args[0]){ 
				...
				case 9 : $sql .= "ORDER BY a.`count` DESC,a.`id` DESC "; break;
				case 10 : $sql .= "ORDER BY a.`count` ASC,a.`id` ASC "; break;
	...
	private static function single($id,$related = 0){
		...
		$sql_public = "SELECT a.*,b.`title` as `channel_title`,b.`pathcategory`,b.`pathcontent`,c.`title` as `category_title` FROM `{$db->prefix}download` a LEFT JOIN `{$db->prefix}channel` b ON a.`channel` = b.`id` LEFT JOIN `{$db->prefix}category` c ON a.`category` = c.`nexus` WHERE a.`lang` = '".LANG."' && a.`audit` = 1 && a.`recycle` = 0 "; 
