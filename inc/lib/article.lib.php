...
class article{
	public static function data(){
		...
		if(empty($args[5])){
			global $db;
			$query_sql = "SELECT a.*,b.`title` as `channel_title`,b.`pathcategory`,b.`pathcontent`,c.`title` as `category_title` FROM `{$db->prefix}article` a LEFT JOIN `{$db->prefix}channel` b ON a.`channel` = b.`id` LEFT JOIN `{$db->prefix}category` c ON a.`category` = c.`nexus` ";
	...
	private static function single($id,$related = 0){
		...
		$sql_public = "SELECT a.*,b.`title` as `channel_title`,b.`pathcategory`,b.`pathcontent`,c.`title` as `category_title` FROM `{$db->prefix}article` a LEFT JOIN `{$db->prefix}channel` b ON a.`channel` = b.`id` LEFT JOIN `{$db->prefix}category` c ON a.`category` = c.`nexus` WHERE a.`lang` = '".LANG."' && a.`published` = 1 && a.`audit` = 1 && a.`recycle` = 0 "; 
