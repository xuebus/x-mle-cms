...
$mle['op_url'] = 'article_manage.php?channel='.(is_numeric($_GET['channel']) ? $_GET['channel'] : 0).'&category='.(is_numeric($_GET['category']) ? $_GET['category'] : 0).'&sort='.(is_numeric($_GET['sort']) ? $_GET['sort'] : 0).'&page='.((is_numeric($_GET['page']) && $_GET['page'] > 1) ? $_GET['page'] : 1).'&trash='.($_GET['trash'] == 1 ? 1 : 0).'&filter='.(is_numeric($_GET['filter']) ? $_GET['filter'] : 0); 
...
if(isset($action) && is_array($ids)){
	...
	switch ($action){
		...
		case 'del' : $log_type = 4; $log_info = $language['page']['volume'][2]; $update = '`recycle` = 1'; comment::delete($ids,'aid',1); break; 
	...
	if($action == 'clear'){ 
		$wait_tag = $db->query("SELECT `tag` FROM `{$DB['prefix']}article` WHERE `id` in ('".implode("','",$ids)."');");
		$tag = '';
		foreach($wait_tag as $n){
			empty($n['tag']) or $tag .= ','.$n['tag'];	
		}
		$tag = explode(',',trim($tag,','));
		tag::delete($tag,'MO001x');
		admin::remove_upload($ids,'MO001x');
		comment::delete($ids,'aid',1);
		$result = $db->delete('article',$ids);
	} else {
		$result = $db->execute("UPDATE `{$DB['prefix']}article` SET {$update} WHERE `id` in ('".implode("','",$ids)."');");			
	}
	if($result === true){
...
$list = article_list($_GET['channel'],$_GET['category'],$_GET['word'],$_GET['sort'],$_GET['filter'],$_GET['trash']);
$mle['list'] = $list['data']; 
...
$sql = "SELECT * FROM `{$DB['prefix']}category` WHERE `lang` = '".LANG."' && `module` = 'MO001x' ";
...
function article_list(){
	global $db,$_GET;
	$param = func_get_args();
	$sql = "SELECT a.*,b.`title` as `channel_title`,c.`title` as `category_title` FROM `{$db->prefix}article` a LEFT JOIN `{$db->prefix}channel` b ON a.`channel` = b.`id` LEFT JOIN `{$db->prefix}category` c ON a.`category` = c.`nexus` WHERE a.`lang` = '".LANG."' ";
